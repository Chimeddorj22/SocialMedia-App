...
class CreateMaterial extends StatefulWidget {
  ...
  const CreateMaterial({Key key, this.id}) : super(key: key);
...
class _CreateMaterialState extends State<CreateMaterial> {
  ...
  continued() {
    _currentStep < 3
        ...
        : setState(() {
            if (_mName != null && _description != null && _image1 != null) {
              buttonText = "Хүсэлт илгээх";
  ...
  setUserIdentityInfo(data, {File image}) async {
    ...
    final docdata = <String, dynamic>{
      ...
      'qrcode': '',
      'createdAt': now.toString().split(".")[0],
      'updatedAt': "no"
    };
    String idid = '1';
    ...
    final storageReference = FirebaseStorage.instance
        .ref()
        .child('Device/docs/${widget.id}/${Path.basename(image.path)}');
    print("2" + idid);
    ...
    await uploadTask.then((value) {
      storageReference.getDownloadURL().then((fileURL) async {
        _deviceCollection
            ...
            .collection('docs')
            .doc(idid)
            .update({'file': fileURL.toString(), 'key': idid});
        print("3" + idid);
      });
    });
    Navigator.push(
      context,
      CupertinoPageRoute(
        builder: (_) => Generate(id: idid, deviceid: widget.id),
      ),
    );
  }
}
